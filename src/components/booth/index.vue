        ...
        <div class="boothIndex topSeat botSeat">
            <div class="sticky-area">
                <div class="booth-fav-tab">
                    <ul>
                        <li @click="searchRoute"><span title="展品">{{$t("Exhibits.Title")}}</span></li>
                        <li class="active"><span title="展位">{{$t("HomePage.Exhibitors")}}</span></li>
                    </ul>
                </div>
                <div class="cell-filter">
                    <div class="cell-filter-box">
                        <ul>
                            ...
                            <li title="新展商" @click="changeSearchTag($event,'new')">{{$t("label.newBooth")}}</li>
            ...
            <div class="boothList" v-show="!showAddress && !showBoothClassifyFlag">
                <v-load :loadDownFn = "loadDownFn" :loadUpFn='loadUpFn' ref='vLoad'> 
                    <slot>
                        <boothList :tochildlist='listArry' :mapLink='mapLink' :showHot="'true'"></boothList>
                    </slot>
                </v-load>
            </div>
            <!--分类筛选-->
    ...
    methods: {
        addHotBooth: function(arr, num) {
            ...
            try {
                ...
                if (_max != _min && arr.length >= pushPlace && this.tuijianList.length >= 2) {
                    //此区间能添加推荐
                    let arrPre2 = this.tuijianList.slice(0, 2); //推荐截取前两个
                    this.tuijianList.splice(0, 2)
        ...
        _getList: function(num, me, type) {
            ...
            //推荐展位列表接口
            //先要判断 tuijianList 小于2 则重启
            let getHotBooth = '';
            if(_this.tuijianList.length < 2) {
                
                let _lan = this.$cookie.get('language') || 'ch';//当前cookie中的中英文
                getHotBooth = commonAjaxFun({
            ...
            let promiseAll = Promise.all([getBooths, getHotBooth]);//全部ajax
            ...
            promiseAll.then((data) => {
                ...
                try {
                    ...
                    if (normalData.state == '10012' && _this.searchTag == 'local') {
                    ...
                    }else if(normalData.state == "10001"){
                        me.lock();
                        me.noData();
                        me.resetload();
                        ...
                        let backAddHotArr = _this.addHotBooth(_data.pageObj.rows, num);
            ...
            commonAjaxFun({
                ...
                try {
                    ...
                    if (data.state == '0') {
                        ...
                        if (_data.length > 0) {
                            _this.hasMoreTuijianFlag = true; //还有更多展商，可以在加载
                        } else {
                            _this.hasMoreTuijianFlag = false; //没有更多展商了，不可以在加载
                        }
